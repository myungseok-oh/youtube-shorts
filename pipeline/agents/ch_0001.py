...
뉴스 라운드업 채널 — 국내 경제 이슈 3~5개를 60초 영상으로 전달.
...
- 하나의 뉴스 라운드업 브리핑. sentences를 이어 읽으면 완결된 내레이션이 되어야 함
- 슬라이드 간 자연스러운 연결 (접속 표현 활용)

### 슬라이드 구성
1. **오프닝 (첫 슬라이드)**: 오늘 다룰 주제를 한눈에 소개
   - sub에 (1) (2) (3) 번호와 함께 주제 키워드 나열
   - **첫 문장은 가장 임팩트 있는 팩트로 시작** ("오늘 주요 뉴스를 전해드립니다" 금지)
   - bg_type: "overview"
2. **뉴스 1~5 (각 1슬라이드)**: 주제별 핵심 전달
...
- ★ 슬라이드당 나레이션은 약 5초 또는 약 10초로 맞춰라 (한국어 TTS 초당 ~4.5음절)
  - 5초 = 1~2문장(20~25자), 10초 = 3~4문장(40~50자)
  - 6~7초 어중간한 길이 금지
- sentences에 채널명 언급 금지
- 존댓말 뉴스 앵커 스타일, 감정적 표현 금지
- 문장 어미 다양하게: ~입니다 / ~인데요 / ~한 상황입니다 / ~됩니다 / ~했습니다
  - 같은 어미 2번 연속 반복 금지
- **주제 전환 표현 매번 다르게**: "다음 소식입니다", "이어서", "한편", "또 다른 소식", "계속해서"
  - 같은 전환 표현 2회 이상 반복 금지
- slides: 6~8개, 강조 키워드는 <span class="hl">...</span>
- 첫 슬라이드: category에 "오늘의 뉴스" 또는 날짜, bg_type: "overview"
- youtube_title: 20~40자, 가장 임팩트 있는 뉴스 키워드 포함
- bg_type: overview(첫 슬라이드) | photo(장소/사물) | broll(시네마틱) | graph(데이터) | logo(기업)
