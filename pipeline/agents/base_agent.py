...
class BaseAgent:
    ...
    SCRIPT_RULES = """\
- news_date: 참조 자료/기사의 게시 날짜 또는 제작일 (YYYY-MM-DD), 정확히 기입
- 하나의 주제를 담은 영상. sentences를 이어 읽으면 완결된 내레이션이 되어야 함
- 슬라이드 간 자연스러운 연결 (접속 표현 활용)
- 슬라이드별 역할: ①훅 → ②핵심 → ③배경 → ④영향 → ⑤전망 (주제 성격에 맞게 변형)
- ★ 슬라이드당 5초 또는 10초로 맞춰라 (어중간한 6~7초 금지)
  - 5초 = 1~2문장(20~25자), 10초 = 3~4문장(40~50자)
  - 슬라이드 수와 문장 수는 채널 지침의 영상 길이에 맞춰 조절
- sentences에 채널명 언급 금지
- ★ 톤/말투/문장 종결 어미는 채널 지침의 "말투 규칙"을 절대적으로 따른다
  (채널이 친근체 지시하면 "~니다" 같은 뉴스 앵커체 절대 사용 금지)
...
### 슬라이드 구성 (필수)
**주제별 슬라이드 (각 1개씩)**: 각 뉴스를 1슬라이드로 요약
   - category: "1️⃣ 경제" / "2️⃣ 국제" 등 번호 + 분야
   - main: 핵심 내용 (키워드 강조)
   - sub: 보조 설명 1줄
   - sentences: 각 주제당 2~3문장 (핵심 팩트만)
   - bg_type: 주제에 맞게 photo/graph/logo 선택
- ★ 오프닝 안내(주제 목록 소개)는 채널 인트로 세그먼트가 처리하므로 첫 슬라이드부터 곧바로 첫 번째 주제로 시작

### 대본 규칙
- 문장 수와 슬라이드 수는 채널 지침의 영상 길이에 맞춰 조절
...
- bg_type: photo(장소/사물), broll(시네마틱), graph(데이터), logo(기업)"""
        ...
        if _is_specific_topic(request):
            ...
            try:
                raw = _run_claude(check_prompt, timeout=60,
                                  model="claude-sonnet-4-6", use_web=True)
...
"""
        ...
        raw = _run_claude(prompt, timeout=900, model="claude-opus-4-6",
...
- ★ 영상 길이/슬라이드 수/문장 수는 채널 지침의 대본 규칙을 따를 것
- 각 씬의 역할(role): hook(시작 훅) / context(배경) / detail(구체 내용) / impact(영향) / outlook(전망)
- 라운드업이면: topic_1 / topic_2 / ... 형태 (오프닝 안내는 채널 인트로 세그먼트가 처리)
    ...
    def generate_visual_plan(self, topic: str, synopsis: dict,
        ...
        if bg_media_type == "single":
            _vp_duration_media = (
                ...
                f"- graph 타입은 항상 \"image\"\n"
                f"- 첫 씬은 시청자 주의를 끄는 강렬한 비주얼"
            )
            _vp_duration_rules = (
                f"1. **duration 결정이 핵심**: 모든 씬의 duration 합계가 채널 지침의 영상 길이에 맞도록 배분\n"
...
★ 영상 길이는 채널 지침에 명시된 값을 따른다 (closing 제외 duration 합계가 이 범위에 맞아야 함)
...
- 한국어 TTS 기준: 초당 ~4.5음절 → duration × 4.5 = 최대 음절 수
- 예: 5초 씬 → ~22음절, 6초 씬 → ~27음절, 10초 씬 → ~45음절
...
{{
  ...
  "intro_narration": "도입부 1~2문장 멘트 (60~120자, 핵심 헤드라인 미리보기)",
  "hashtags": ["채널 지침 해시태그 + 내용 관련 태그 5개 (# 없이)"],
  "sentences": [
    {{"text": "첫번째 슬라이드 팩트 문장 (15~25자)", "slide": 1}},
    {{"text": "첫번째 슬라이드 원인/배경 문장", "slide": 1}},
    {{"text": "두번째 슬라이드 팩트 문장", "slide": 2}},
    {{"text": "두번째 슬라이드 원인/배경 문장", "slide": 2}},
    {{"text": "세번째 슬라이드 팩트 문장", "slide": 3}},
    {{"text": "세번째 슬라이드 원인 문장", "slide": 3}},
    {{"text": "세번째 슬라이드 영향 문장", "slide": 3}}
  ],
  "slides": [
    {{"category": "카테고리", "main": "수치 <span class=\\"hl\\">강조</span>", "sub": "원인 설명", "bg_type": "photo"}},
    {{"category": "카테고리", "main": "수치 <span class=\\"hl\\">강조</span>", "sub": "원인 설명", "bg_type": "photo"}},
    ...
  ]
}}
...
★ hashtags: 채널 지침의 ## 해시태그 섹션에 명시된 고정 태그 + 이 영상 내용에 맞는 태그 5개 추가. # 기호 없이 문자열 배열로.

★★★ intro_narration 작성 규칙 (영상 도입부 인트로 세그먼트에서 읽힐 멘트):
- 1~2문장, 60~120자, 어떤 내용을 다루는지 미리 알려주는 톤
- 라운드업이면: 반드시 2문장으로 분리. 키워드 사이는 쉼표(,)로 띄운다. 가운뎃점(·) 사용 금지 — TTS가 휴지 없이 붙여 읽어 급해 보임
  - 예: "이란 협상 교착 등 오늘의 핵심 뉴스 8건입니다. 전기차, 연금, 반도체 이슈까지 차례로 정리했습니다."
- 단일 주제이면: 핵심 팩트 + 호기심 유발 한 줄
  - 예: "비트코인 사상 최고가 돌파, 지금 무슨 일이 벌어지고 있는지 정리했습니다."
- 채널명/날짜/요일 언급 금지 (별도 자막 처리)
- 본문 sentences와 단어 그대로 반복 금지 (압축 키워드만)
- "오늘의 뉴스입니다" 같은 빈 멘트 금지 — 반드시 실제 키워드 포함
- "빠르게" 등 급한 느낌 주는 부사 지양
    ...
    def _validate_with_claude(self, draft_json: dict, instructions: str,
        ...
        if bg_media_type == "single":
            _duration_media_block = (
                f"### duration/media\n"
                f"- duration 합계는 채널 지침의 영상 길이를 따른다. 모든 씬 5초 또는 10초 (정보 많은 씬만 10초)\n"
                f"- ★★★ 슬라이드 1개 = visual_plan 1개 (1:1 매핑). 슬라이드당 2개 이상 생성 절대 금지.\n"
                f"- ★★★ 모든 씬의 media를 반드시 \"image\"로 지정. video 생성 금지. motion은 빈 문자열.\n"
                f"- graph → 항상 image\n"
                f"- bg_type: photo/graph/logo.\n"
                f"- ★ closing 슬라이드는 생성하지 마라 (시스템이 자동 처리)"
            )
        elif bg_media_type == "auto":
            _duration_media_block = (
                f"### duration/media\n"
                f"- duration 합계는 채널 지침의 영상 길이를 따른다. image: 5초(정보 많으면 10초, 최대 1~2개). video: 6초 기본. 6~7초 image 금지\n"
                f"- 이미지:영상 비율 약 6:4 (전체 씬 중 ~40%를 'video'로 지정)\n"
                f"- 동적 장면(행동/움직임/변화)은 video, 정적 장면(설명/도입/정리)은 image\n"
                f"- graph → 항상 image. video의 en에 움직임 키워드 포함\n"
                f"- motion: 카메라+피사체+환경 조합 (예: \"slow zoom in on factory as smoke rises\")\n"
                f"- bg_type: photo/broll/graph/logo.\n"
                f"- ★ closing 슬라이드는 생성하지 마라 (시스템이 자동 처리)"
            )
        else:
            _duration_media_block = (
                f"### duration/media\n"
                f"- duration 합계는 채널 지침의 영상 길이를 따른다. image: 5초(정보 많으면 10초). video: 6초 기본\n"
...
2. **★ 슬라이드당 최소 2문장 강제**: 1문장만 있는 슬라이드가 있으면 반드시 원인/배경/영향 문장을 추가하여 2~3문장으로 보강. 이 규칙은 "불필요한 변경 금지"보다 우선한다.
3. **텍스트 길이**: main 20자 이내, sub 25자 이내. 넘으면 축약. <span class="hl">...</span>으로 강조
4. **나레이션 분량**: 위 대본 규칙의 분량 기준과 duration이 매칭되는지
5. **나레이션 흐름**: sentences를 이어 읽으면 연속된 내레이션이 되어야 함. 슬라이드 간 인과 연결, 접속 표현 활용
6. **채널 톤 준수**: 채널 지침(위 instructions)의 톤/스타일에 맞는지. 문장 종결 다양하게
7. 위 1~6 항목에 문제 없으면 그대로 유지 (불필요한 변경 금지)
...
→ 예: "48시간 타이머" → 모래시계 / "앱 채팅창" → 펜과 메모장 / "10% 수치" → 계산기와 영수증

### ★ 이미지 로컬라이즈 규칙 (나레이션의 발생 지역 기준) ★
- ★ 나레이션을 먼저 읽고 사건의 발생 지역을 파악한 뒤 그에 맞는 환경을 프롬프트에 반영한다.
...
- 해외 사건: 나레이션에 명시된 국가/지역 환경 사용
  ...
  - 미국: American setting, Western infrastructure, English signage
...
- ★ 절대 금지: 나레이션의 지역 정보를 무시하고 임의로 Korean을 강제하지 말 것 (예: 이란 뉴스에 Korean setting은 부조리)
...
{{
  ...
  "script": {{
    "news_date": "...",
    "youtube_title": "...",
    "intro_narration": "도입부 1~2문장 멘트 (60~120자, 헤드라인 미리보기)",
    "hashtags": ["채널 지침 해시태그 + 내용 관련 태그 5개 (# 없이)"],
    "sentences": [
      {{"text": "슬라이드1 팩트 문장", "slide": 1}},
...
★ hashtags: 채널 지침의 ## 해시태그 섹션에 명시된 고정 태그 + 이 영상 내용에 맞는 태그 5개 추가. # 기호 없이 문자열 배열로.
★ 각 슬라이드에 반드시 2~3개 문장 배정. 1문장만 있는 슬라이드는 규칙 위반.
★ intro_narration 작성 규칙:
  - 영상 도입부 인트로 세그먼트(헤드라인 카드 등장)에서 읽힐 멘트 (1~2문장, 60~120자)
  - 라운드업이면: 반드시 2문장으로 분리. 키워드 사이는 쉼표(,)로 띄운다. 가운뎃점(·) 금지 — TTS가 붙여 읽어 급해 보임
    - 예: "이란 협상 교착 등 오늘의 핵심 뉴스 8건입니다. 전기차, 연금, 반도체 이슈까지 차례로 정리했습니다."
  - 단일 주제이면: 핵심 팩트 + 호기심 유발 한 줄
  - 채널명/날짜/요일 언급 금지, 본문 sentences와 단어 그대로 반복 금지
  - "빠르게" 등 급한 느낌 주는 부사 지양
"""
        raw = _run_claude(prompt, timeout=300, model="claude-sonnet-4-6",
        ...
        if "sentences" in validated and "slides" in validated and "script" not in validated:
            validated = {
                "script": {
                    ...
                    "intro_narration": validated.pop("intro_narration", script.get("intro_narration", "")),
...
{topic_display}"""
        ...
        if bg_media_type == "single":
            ...
            _duration_media_block = (
                ...
                f"- graph → 항상 image. video의 en에 움직임 키워드 포함\n"
                f"- motion: 카메라+피사체+환경 조합 (예: \"slow zoom in on factory as smoke rises\")\n"
                f"- bg_type: photo/broll/graph/logo. (단일 형식 사용 금지)\n"
...
### 시놉시스 규칙
- 영상 길이는 채널 지침의 대본 규칙을 따를 것 (슬라이드/문장 수 포함)
...
- 스마트폰 화면, 앱 UI, 채팅창, 알림 화면 등 디지털 인터페이스 렌더링 금지
- chart, bar graph, infographic, diagram 등 차트/그래프 생성 금지
- text, numbers, labels 등 이미지 내 텍스트/숫자 렌더링 금지
- Dialogue, lip movements, gestures 등 대사/동작 지시 금지
→ 수치/통계는 나레이션으로 전달, 배경은 실제 사물/장소 사진으로 표현
...
- 한국어 TTS 기준: 초당 ~4.5음절 → 각 씬의 duration × 4.5 = 최대 음절 수
- 예: 5초 씬 → ~22음절, 6초 씬 → ~27음절, 8초 씬 → ~36음절, 10초 씬 → ~45음절
- ★ 음절 수를 초과하면 나레이션이 배경보다 길어진다. 절대 초과 금지.
...
"""
        ...
        if not gemini_api_key:
            ...
            raw = _run_claude(prompt, timeout=_timeout, model="claude-opus-4-6",
                              retries=1,
...
- text, numbers, labels 등 이미지 내 텍스트/숫자 렌더링 금지
- Dialogue, lip movements, gestures 등 대사/동작 지시 금지
- 추상적 의료/과학 다이어그램, 신경 네트워크 모식도 금지
→ 수치/통계는 나레이션으로 전달하고, 배경은 해당 주제의 실제 사물/장소 사진으로 표현
→ 예: "48시간 타이머" → 모래시계 클로즈업 / "채팅 문의" → 펜과 메모장 / "앱 화면" → 노트북이 있는 책상
