...
{{
  ...
  "intro_narration": "도입부 1~2문장 멘트 (60~120자, 어떤 내용을 다루는지 미리보기)",
  "sentences": [
...
- 영상 도입부 인트로 세그먼트에서 읽힐 짧은 멘트 (1~2문장, 60~120자)
- 어떤 내용을 다루는지 한 줄로 미리 알려주는 톤 (호기심 유발 + 시청 유도)
- 채널명/날짜/요일 언급 금지 (날짜는 별도 자막)
- 본문 sentences와 단어 중복 최소화
- 단순 "이번 영상에서 알려드립니다" 식 빈 멘트 금지 — 핵심 키워드 1~2개 포함
"""
...
{{
  ...
  "sentences": [
    ...
    {{"text": "섹션1 팩트 문장", "slide": 1}},
    {{"text": "섹션1 원인/배경 문장", "slide": 1}},
    {{"text": "섹션2 팩트 문장", "slide": 2}},
    {{"text": "섹션2 원인 문장", "slide": 2}},
    {{"text": "섹션2 영향 문장", "slide": 2}}
  ],
  "slides": [
    {{
      "category": "1️⃣ 카테고리",
...
intro_narration 작성 규칙 (★ 라운드업 인트로 카드와 연결):
- 영상 도입부 인트로 세그먼트(헤드라인 카드 등장)에서 읽힐 멘트 (1~2문장, 60~120자)
- 첫 번째 헤드라인의 핵심 팩트로 시작 + 다른 헤드라인의 핵심 키워드 2~4개를 가운뎃점(·)으로 나열
- 톤: "{{핵심 헤드라인}} 포함 N개 핵심 뉴스 정리했습니다. {{키워드1}}·{{키워드2}}·{{키워드3}} 이슈까지 빠르게 짚습니다." 같은 패턴
- 채널명/날짜/요일 언급 금지 (별도 자막에서 처리)
- 본문 sentences와 같은 어구 그대로 반복 금지 — 압축된 키워드만
- "오늘의 헤드라인입니다" 같은 빈 멘트 금지 — 반드시 실제 키워드 포함
- 예: "이란 협상 교착 포함 8개 핵심 뉴스 정리했습니다. 전기차·연금·반도체 이슈까지 빠르게 짚습니다."
"""
