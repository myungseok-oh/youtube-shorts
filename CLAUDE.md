...
6. **⚠️ 자막 위치/크기 로직 수정 금지**: `_render_sub_png()`, `apply_subtitles()`, CSS 미리보기(`app.js updateSlidePreview` 내 pvSub) 세 곳이 1:1 대응하는 구조. 하나만 바꾸면 미리보기≠실제 렌더링 불일치 발생. **반드시 사용자 확인 후 세 곳 동시에 수정**할 것. (Pillow px = CSS px, margin_v = CSS bottom/top)
...
## 채널 목록
- **이시각 헤드라인** (ch-0001) — 종합 뉴스, 기본 채널
- **1분 심리학** (ch-0002) — 심리학 교양 콘텐츠
- **30초 상식** (ch-0003) — 짧은 상식 교양
- **30초 뉴스** (ch-0004) — 짧은 뉴스
- **증시/코인** (ch-0005) — 시장 브리핑
- **동물 심리학** (ch-0006) — 반려동물 행동 심리학 교양 콘텐츠
